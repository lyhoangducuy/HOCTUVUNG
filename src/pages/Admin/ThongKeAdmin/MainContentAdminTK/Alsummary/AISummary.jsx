...
const analyzeDataWithAI = async (data) => {
  ...
  const prompt = `
Hãy phân tích dữ liệu sau và trả lời thật NGẮN GỌN (tối đa 3 câu), chỉ nêu ý chính.
...
Trả về kết quả bằng tiếng Việt, có cấu trúc rõ ràng với các emoji phù hợp phải cực kì ngắn gọn nhưng trình bày đặc sắc xuống dòng đoàng hoàng.
`;
...
export default function AISummary({ users = [], classes = [], cards = [], userStats = [], revenue = {} }) {
  ...
  const generateAIAnalysis = async () => {
    ...
    try {
      ...
      setError("Không thể kết nối AI. Vui lòng kiểm tra API key hoặc thử lại sau.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const fallbackAnalysis = useMemo(() => {
    ...
    if (monthRevenue > 0) {
      insights.push(`💰 Doanh thu tháng: ${monthRevenue.toLocaleString('vi-VN')} VNĐ`);
    }
    
    if (totalRevenue > 0) {
      insights.push(`💎 Tổng doanh thu: ${totalRevenue.toLocaleString('vi-VN')} VNĐ`);
    }
  ...
  return (
    ...
    {loading ? (
      <div className="aisummary-loading">
        <FaSpinner className="spin" style={{ marginRight: 12, fontSize: 20 }} />
